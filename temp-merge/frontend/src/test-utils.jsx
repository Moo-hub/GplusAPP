...
export function runGenericScreenTests(ComponentOrElement, {
  ...
  describe(`${displayName} tests`, () => {
    ...
    it('renders loading state', () => {
      ...
      render(el);
      // tolerate multiple loading nodes (React StrictMode double-mount)
      const loadingNodes = screen.getAllByTestId(loadingTestId);
  ...
  const hasError = screen.queryAllByTestId('error').length > 0;
    ...
    if (emptyKey) {
      it('renders empty state', async () => {
        ...
        if (!e.length && !gs.length && !erEl.length && emptyKey) {
    ...
    if (errorKey) {
      it('renders error state', async () => {
        ...
        if (!er.length && !gs2.length && !empty2.length && errorKey) {
...
export const setupI18nMock = (additionalTranslations = {}) => {
  ...
  return {
    useTranslation: () => ({
      t: (key, options) => {
        // Support dot notation for nested keys (arbitrary depth)
        if (key && key.includes('.')) {
