    ...
    const fetchData = async () => {
      ...
      try {
        if (activeTab === 'personal') {
          ...
          const personalResponse = await axios.get(`/api/v1/environmental-impact/user/${userId}/summary`);
          setPersonalData(personalResponse.data);
          
          // Fetch personal trend data
          const trendResponse = await axios.get(
            `/api/v1/environmental-impact/trend?metric=carbon&time_range=${timeRange}&granularity=day`
          );
          setTrendData(trendResponse.data);
        } else if (activeTab === 'community') {
          // Fetch community impact data
          const communityResponse = await axios.get(
            `/api/v1/environmental-impact/community?time_period=${timeRange}`
          );
          setCommunityData(communityResponse.data);
          
          // Fetch leaderboard data
          const leaderboardResponse = await axios.get(
            `/api/v1/environmental-impact/leaderboard?time_period=${timeRange}&metric=carbon&limit=10`
          );
