...
import { beforeAll, afterEach, afterAll, vi, expect as vitestExpected } from "vitest";
import { cleanup } from "@testing-library/react";
import * as matchers from "@testing-library/jest-dom/matchers";

// Use conditional import for server to avoid conflicts
let mockServer = null;
try {
  // Use dynamic require to avoid module conflicts
  const serverModule = require("./mocks/server");
  mockServer = serverModule.server;
} catch (e) {
  console.warn('MSW server not available in this test environment');
}
...
if (typeof globalThis.expect === 'undefined' || globalThis.expect !== vitestExpected) {
  // @ts-ignore
  globalThis.expect = vitestExpected;
}
// @ts-ignore
