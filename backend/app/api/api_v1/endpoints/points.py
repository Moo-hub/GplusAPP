...
async def get_environmental_impact(
    ...
    try:
        # Calculate based on user's points and activities
        points = current_user.points or 0
        
        # Simple calculation based on points (real app would use detailed tracking)
        carbon_saved = points * 0.006  # kg CO2 per point
        waste_recycled = points * 0.01  # kg waste per point
        trees_equivalent = carbon_saved / 22.0  # 22kg CO2 per tree annually
        energy_saved = points * 0.12  # kWh per point
        water_saved = points * 0.63  # liters per point
        
        impact_data = {
            "carbonSaved": round(carbon_saved, 2),
            "carbonUnit": "kg CO₂",
            "wasteRecycled": round(waste_recycled, 2),
            "wasteUnit": "kg",
            "treesEquivalent": round(trees_equivalent, 2),
            "energySaved": round(energy_saved, 2),
            "energyUnit": "kWh",
            "waterSaved": round(water_saved, 2),
            "waterUnit": "liters",
            "thisMonth": {
                "carbonSaved": round(carbon_saved * 0.15, 2),  # ~15% this month
                "wasteRecycled": round(waste_recycled * 0.15, 2),
                "treesEquivalent": round(trees_equivalent * 0.15, 2)
            },
            "streak": 7,  # Would calculate from actual activity data
            "rank": "Silver" if points > 500 else "Bronze",
            "totalActivities": max(1, int(points / 25)),  # ~25 points per activity
            "lastActivity": "2024-10-12T10:00:00Z"
        }
        ...
        raise HTTPException(
            status_code=500, 
            detail=f"Failed to get environmental impact: {str(e)}"
        )
