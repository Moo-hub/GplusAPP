...
@router.get("/{notification_id:int}", response_model=Notification)
async def get_notification(
    ...
    if notification.user_id != current_user.id:
        # Return 404 to avoid leaking existence of resources across users
        raise HTTPException(status_code=404, detail="Notification not found")
        
    return notification

@router.patch("/{notification_id:int}", response_model=Notification)
