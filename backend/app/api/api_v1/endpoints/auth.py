...
def login(
    ...
    # دعم مرن: إذا كان username يبدو كإيميل استخدمه كـ email، وإلا جرب البحث بالاسم أو الإيميل
    login_id = form_data.username
    user = None
    if "@" in login_id:
        user = authenticate(db, email=login_id, password=form_data.password)
    else:
        # جرب البحث بالاسم أو الإيميل (للتوافق مع أي واجهة)
        user = authenticate(db, email=login_id, password=form_data.password)
...
@router.post("/verify-email-fix", status_code=status.HTTP_200_OK)
def verify_email_fixed(
    request: Request,
    ...
    (This is a temporary fix for the duplicate endpoint issue).
    """
    # Verify token
    from app.utils.email import verify_token
...
@router.get("/verification-status-fix", response_model=UserVerificationStatus)
def get_verification_status_fixed(current_user: User = Depends(get_current_user)) -> UserVerificationStatus:
    """
