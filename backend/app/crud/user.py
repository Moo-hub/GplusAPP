...
def authenticate(db: Session, email: str, password: str) -> Optional[User]:
    # دعم مرن: جرب البحث بالإيميل أولاً، ثم بالاسم إذا لم يوجد
    user = get_by_email(db, email=email)
    if not user:
        # جرب البحث بالاسم (للتوافق مع بعض الواجهات)
        user = db.query(User).filter(User.name == email).first()
        if not user:
            print(f"[AUTH DEBUG] No user found with email or name: {email}")
            return None
    if not verify_password(password, user.hashed_password):
        print(f"[AUTH DEBUG] Password mismatch for user: {email}")
        print(f"[AUTH DEBUG] Provided password: {password}")
        print(f"[AUTH DEBUG] Hashed in DB: {user.hashed_password}")
        return None
    print(f"[AUTH DEBUG] Authentication success for user: {email}")
    return user
